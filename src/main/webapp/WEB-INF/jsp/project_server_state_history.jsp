...
<br><br><br><br>
<font size="5" style="margin-left: 40%">Server states history</font>
<div class="container" style="margin-top: -100px">
    ...
    <div class="component">
        <table>
            ...
            <c:forEach items="${states}" var="temp" varStatus="st">
                <tr>
...
<div style="text-align:center ">
    <a href="/server/getProjectServerStateHistory.do?projectId=<c:choose><c:when test="${states == null || fn:length(states) == 0}">0</c:when><c:otherwise>${states.get(0).project_id}</c:otherwise></c:choose>&&start=0">first</a>&nbsp;&nbsp;
    <a href="/server/getProjectServerStateHistory.do?projectId=<c:choose><c:when test="${states == null || fn:length(states) == 0}">0</c:when><c:otherwise>${states.get(0).project_id}</c:otherwise></c:choose>&&start=<c:choose><c:when test="${start-10<0}">0</c:when><c:otherwise>${start-10}</c:otherwise></c:choose>">prev</a>&nbsp;&nbsp;
    <a href="/server/getProjectServerStateHistory.do?projectId=<c:choose><c:when test="${states == null || fn:length(states) == 0}">0</c:when><c:otherwise>${states.get(0).project_id}</c:otherwise></c:choose>&&start=<c:choose><c:when test="${start+10>lastStart}">${lastStart}</c:when><c:otherwise>${start+10}</c:otherwise></c:choose>">next</a>&nbsp;&nbsp;
    <a href="/server/getProjectServerStateHistory.do?projectId=<c:choose><c:when test="${states == null || fn:length(states) == 0}">0</c:when><c:otherwise>${states.get(0).project_id}</c:otherwise></c:choose>&&start=${lastStart}">last</a>
</div>
